...
(function(){
  ...
  window.exportProjectDocx = async function(p, opts){
    try{
      ...
      // Support optional as-of date for snapshot fields
      const asOf = (opts && opts.asOfDate) ? String(opts.asOfDate).slice(0,10) : '';
      function byDateAsc(a,b){ return new Date(a.date||'1970-01-01') - new Date(b.date||'1970-01-01'); }
      function getSnapshotAcc(project, asOfYmd){
        const list = (project.accomplishments || []).slice().sort(byDateAsc);
        if(list.length === 0) return {};
        if(!asOfYmd){ return list[list.length-1]; }
        // Find the last item with date <= asOf
        let pick = null;
        for(const a of list){ if((a.date||'') <= asOfYmd){ pick = a; } }
        return pick || list[0]; // fallback to earliest if all are after asOf
      }
      const latest = getSnapshotAcc(p, asOf);
      const durationText = `${p.originalDuration || 0} days${p.timeExtension ? ' +' + p.timeExtension : ''}`;
      const photos = (p.photos && p.photos.length) ? p.photos : (p.sCurveDataUrl ? [p.sCurveDataUrl] : []);
      try{ console.info('[export] Photos available:', (photos||[]).length); }catch(_){/* ignore */}
      const altImages = {
      ...
      const allAcc = (p.accomplishments || []).slice().sort(byDateAsc);
      const accUpToAsOf = asOf ? allAcc.filter(a=> (a.date||'') <= asOf) : allAcc;
      const data = {
        ProjectName: valOrNA(p.name),
        ImplementingAgency: valOrNA(p.implementingAgency),
        Contractor: valOrNA(p.contractor),
        ...
        PercentPlanned: (latest.plannedPercent ?? 0).toFixed(2) + '%',
        PercentPrevious: (latest.prevPercent ?? 0).toFixed(2) + '%',
        AsOfDate: valOrNA(latest.date || asOf),
        Issues: valOrNA(p.issues),
        ...
        accomplishments: accUpToAsOf.map(a=>({
          date: valOrNA(a.date),
          plannedPercent: (a.plannedPercent ?? 0).toFixed(2) + '%',
          prevPercent: (a.prevPercent ?? 0).toFixed(2) + '%',
          percent: (a.percent ?? 0).toFixed(2) + '%',
